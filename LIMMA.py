...
class LinuxSystemControl:
    def __init__(self):
        self.required_tools = {
            ...
            "Diğer": ["steghide", "stegdetect", "tensorflow", "mythril", "prowler", 
                     "cuckoo", "filebeat", "osquery", "lynis"]
        }
    ...
    def safe_file_operation(self, file_path: str, mode: str = 'r') -> Optional[str]:
    ...
    def get_package_manager(self):
        ...
        try:
            if shutil.which('apt'):
                ...
                print(f"{Fore.RED}Desteklenen paket yöneticisi bulunamadı.{Style.RESET_ALL}")
                return None
    ...
    def install_package(self, package_name):
        ...
        try:
            ...
            if pkg_manager:
                ...
                if result.returncode == 0:
                    print(f"{Fore.GREEN}{package_name} başarıyla yüklendi.{Style.RESET_ALL}")
                    return True
                else:
                    print(f"{Fore.RED}{package_name} yüklenemedi: {result.stderr}{Style.RESET_ALL}")
            return False
        except Exception as e:
    ...
    def check_platform(cls):
        ...
        if platform.system() != "Linux":
            print(f"{Fore.RED}Bu program yalnızca Linux sistemlerde çalışır.{Style.RESET_ALL}")
            sys.exit(1)
    ...
    def check_tool(self, tool_name: str) -> bool:
        ...
        if not shutil.which(tool_name):
            ...
            if input(f"{tool_name} otomatik olarak kurulsun mu? (e/h): ").lower() == 'e':
    ...
    def error_handler(func):
        ...
        def wrapper(self, *args, **kwargs):
            try:
                ...
                print(f"{Fore.RED}{func.__name__} çalıştırılırken hata: {str(e)}{Style.RESET_ALL}")
                return None
        return wrapper
    ...
    def cleanup(self):
        ...
        try:
            ...
            try:
                ...
                processes = self.run_command("ps aux | grep -i 'nc\\|netcat' | grep -v 'grep'")
                if processes:
                    ...
                    while True:
                        ...
                        if 0 <= tool_idx < len(missing_tools):
                            ...
                            if install_choice == "1":
                                ...
                                print(f"{Fore.RED}Geçersiz kurulum seçimi!{Style.RESET_ALL}")
                                continue
                                
                            print(f"\n{Fore.YELLOW}{tool_to_install} kuruluyor...{Style.RESET_ALL}")
                            if self.run_command(cmd, sudo=True):
                                print(f"{Fore.GREEN}{tool_to_install} başarıyla yüklendi.{Style.RESET_ALL}")
                                missing_tools.remove(tool_to_install)
                                if not missing_tools:
                                    del missing_by_category[selected_category]
                                    break
                            else:
                                print(f"{Fore.RED}{tool_to_install} yüklenemedi!{Style.RESET_ALL}")
                        else:
    ...
    def network_interfaces(self):
        ...
        try:
            ...
            print(f"{Fore.RED}Ağ arayüzleri kontrolünde hata: {str(e)}{Style.RESET_ALL}")
    ...
    def list_users(self):
        ...
        try:
            ...
            print(f"{Fore.RED}Kullanıcı listesi alınırken hata: {str(e)}{Style.RESET_ALL}")
    ...
    def vulnerability_scan(self):
        ...
        if self.check_tool("openvas"):
            ...
            output_dir = f"vulnerability_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            if scan_type == "3":
                ...
                custom_options = f"--ports {port_range} --intensity {intensity} --timeout {timeout}"
            else:
                custom_options = ""
                
           
            email_notification = input("\nTarama sonuçları e-posta ile gönderilsin mi? (e/h): ").lower()
            ...
            scheduled_scan = input("\nTarama zamanlanmış olarak çalışsın mı? (e/h): ").lower()
            if scheduled_scan == 'e':
    ...
    def malware_scan(self):
        ...
        if self.check_tool("clamav"):
            ...
            max_scan_size = input("Maksimum tarama boyutu (MB) [Enter=4000]: ") or "4000"
            command += f" --max-filesize={max_file_size}M --max-scansize={max_scan_size}M"

            
            if input("\nBulunan zararlı yazılımlar otomatik temizlensin mi? (e/h): ").lower() == 'e':
                command += " --remove"
            ...
            if email_notification == 'e':
                email = input("E-posta adresi: ")
                self.run_command(f"echo 'Tarama sonuçları ektedir.' | mail -s 'Zararlı Yazılım Tarama Raporu' -a {output_dir}/scan_log.txt {email}")
    ...
    def rootkit_scan(self):
        ...
        if self.check_tool("rkhunter"):
            ...
            if scan_type == "3":
                print("\nÖzel Tarama Seçenekleri:")
                check_network = input("Ağ arayüzlerini kontrol et (e/h): ").lower() == 'e'
                check_ports = input("Açık portları kontrol et (e/h): ").lower() == 'e'
                check_rootkits = input("Bilinen rootkitleri kontrol et (e/h): ").lower() == 'e'
                check_files = input("Şüpheli dosyaları kontrol et (e/h): ").lower() == 'e'
                
                custom_options = ""
                if check_network: custom_options += " --check-network"
                if check_ports: custom_options += " --check-ports"
                if check_rootkits: custom_options += " --check-rootkits"
                if check_files: custom_options += " --check-files"
            else:
                custom_options = ""
            ...
            if result:
                ...
                if input("\nSonuçlar e-posta ile gönderilsin mi? (e/h): ").lower() == 'e':
                    email = input("E-posta adresi: ")
                    self.run_command(f"echo 'Rootkit tarama sonuçları ektedir.' | mail -s 'Rootkit Tarama Raporu' -a {log_file} {email}")
    ...
    def port_scan(self):
        ...
        if self.check_tool("nmap"):
            ...
            # Çıktı dosyası
            save_output = input("Sonuçları dosyaya kaydetmek ister misiniz? (e/h): ").lower()
            output_file = f"nmap_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt" if save_output == 'e' else None
    ...
    def web_scan(self):
        ...
        if self.check_tool("nikto"):
            ...
            if scan_type == "6":
                ...
                dbcheck = input("Veritabanı kontrolü yapılsın mı? (e/h): ").lower() == 'e'
            ...
            output_cmd = " ".join([f"-output {output_dir}/scan_report.{fmt}" for fmt in formats])
                ...
                if input("\nSonuçlar e-posta ile gönderilsin mi? (e/h): ").lower() == 'e':
                    ...
                    for fmt in formats:
                        ...
                        if os.path.exists(report_file):
                            self.run_command(f"echo 'Web tarama sonuçları ektedir.' | mail -s 'Web Tarama Raporu' -a {report_file} {email}")
    ...
    def wifi_scan(self):
        ...
        if self.check_tool("aircrack-ng"):
            ...
            if choice == "1":
                ...
                channel = input("Kanal: ")
                self.run_command(f"airodump-ng -c {channel} --bssid {bssid} -w capture {interface}", sudo=True)
            elif choice == "3":
                self.run_command(f"airodump-ng -c 1 --showack {interface}", sudo=True)
            elif choice == "4":
                self.run_command(f"airodump-ng --manufacturer --uptime {interface}", sudo=True)
    ...
    def arp_scan(self):
        ...
        if self.check_tool("arp-scan"):
            ...
            if choice == "1":
                self.run_command(f"arp-scan --localnet --output={output_file}", sudo=True)
            elif choice == "2":
                ip_range = input("IP aralığı (örn: 192.168.1.0/24): ")
                self.run_command(f"arp-scan {ip_range} --output={output_file}", sudo=True)
            elif choice == "3":
                mac = input("MAC adresi: ")
                self.run_command(f"arp-scan --localnet --macfile={mac} --output={output_file}", sudo=True)
            elif choice == "4":
                vendor = input("Üretici adı: ")
                self.run_command(f"arp-scan --localnet | grep -i {vendor} > {output_file}", sudo=True)
    ...
    def mitm_detection(self):
        ...
        if choice == "1":
            ...
            if self.check_tool("sslscan"):
                ...
                self.run_command(f"sslscan --show-certificate --no-colour {target} > {output_file}")
                
                # Şüpheli sertifikaları kontrol et
                self.run_command(f"cat {output_file} | grep -i 'invalid\\|expired\\|self-signed'")
            ...
            if self.check_tool("tcpdump"):
                ...
                self.run_command(f"tcpdump -i {interface} -w {output_file} -v 'arp or (tcp and port 443)' -G {duration} -W 1", sudo=True)
                
                # Yakalanan paketleri analiz et
                if self.check_tool("wireshark-cli"):
                    self.run_command(f"tshark -r {output_file} -Y 'arp.duplicate-address-detected or ssl.alert.desc == 51'")
        ...
        if input("\nSonuçlar e-posta ile gönderilsin mi? (e/h): ").lower() == 'e':
            email = input("E-posta adresi: ")
            self.run_command(f"tar -czf {output_dir}.tar.gz {output_dir} && echo 'MITM tespit sonuçları ekte.' | mail -s 'MITM Tespit Raporu' -a {output_dir}.tar.gz {email}")
    ...
    def firewall_check(self):
        ...
        if choice == "1":
            ...
            # IPTables kuralları
            self.run_command(f"iptables -L -n -v --line-numbers > {output_dir}/iptables_rules.txt", sudo=True)
            ...
            self.run_command(f"grep UFW /var/log/kern.log | tail -n 50 > {output_dir}/ufw_recent_logs.txt", sudo=True)
            
            # Reddedilen paketleri analiz et
            self.run_command(f"grep 'UFW BLOCK' /var/log/kern.log | awk '{{print $12}}' | sort | uniq -c | sort -nr > {output_dir}/blocked_ips.txt", sudo=True)
            ...
            for config in config_files:
                if os.path.exists(config):
                    ...
                    self.run_command(f"cat {config} > {output_dir}/$(basename {config})", sudo=True)
            
            # Güvenlik önerilerini kontrol et
            recommendations = []
            
            # Default policy kontrolü
            default_policy = self.run_command("ufw status verbose | grep Default:", sudo=True)
        ...
        if input("\nSonuçlar e-posta ile gönderilsin mi? (e/h): ").lower() == 'e':
            email = input("E-posta adresi: ")
            self.run_command(f"tar -czf {output_dir}.tar.gz {output_dir} && echo 'Güvenlik duvarı kontrol sonuçları ekte.' | mail -s 'Güvenlik Duvarı Kontrol Raporu' -a {output_dir}.tar.gz {email}")
    ...
    def network_monitor(self):
        ...
        if choice == "1" and self.check_tool("nethogs"):
            ...
            self.run_command(f"nethogs -t {duration} {interface} > {output_file}", sudo=True)
    ...
    def bandwidth_monitor(self):
        ...
        if choice == "1" and self.check_tool("iftop"):
            ...
            while time.time() < end_time:
                rx_bytes = int(self.run_command(f"cat /sys/class/net/{interface}/statistics/rx_bytes"))
                tx_bytes = int(self.run_command(f"cat /sys/class/net/{interface}/statistics/tx_bytes"))
    ...
    def connection_monitor(self):
        ...
        if choice == "1":
            ...
            suspicious = self.run_command("netstat -tupn | grep -i 'syn_sent\\|established'", sudo=True)
            if suspicious:
            ...
            with open("/etc/services", 'r') as f:
                known_ports = {line.split()[1].split('/')[0] for line in f if line.strip() and not line.startswith('#')}
            
            unknown_ports = set()
            for line in self.run_command("netstat -tlpn | awk '{print $4}'").split('\n'):
                if ':' in line:
                    port = line.split(':')[-1]
            ...
            tcp_stats = self.run_command("netstat -st | grep -i 'failed\\|reset\\|timeout'")
            if tcp_stats:
    ...
    def file_integrity(self):
        ...
        if self.check_tool("aide"):
            if choice == "1":
                ...
                self.run_command(f"aide --check > {output_dir}/full_check.log", sudo=True)
                
            elif choice == "2":
                path = input("Kontrol edilecek dizin: ")
                print(f"\n{Fore.YELLOW}Dizin kontrolü başlatılıyor...{Style.RESET_ALL}")
                self.run_command(f"aide --check --config-check -r {path} > {output_dir}/dir_check.log", sudo=True)
                ...
                while time.time() < end_time:
                    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
                    self.run_command(f"aide --check > {output_dir}/check_{timestamp}.log", sudo=True)
                    time.sleep(float(interval) * 60)
                    
            elif choice == "4":
                print(f"\n{Fore.YELLOW}Rapor oluşturuluyor...{Style.RESET_ALL}")
                self.run_command(f"aide --update > {output_dir}/aide_update.log", sudo=True)
    ...
    def process_monitor(self):
        ...
        if choice == "1" and self.check_tool("htop"):
            ...
            suspicious_patterns = [
                "kworker", "cryptominer", "backdoor",
                "reverse_shell", "netcat", "ncat"
            ]
    ...
    def user_audit(self):
        ...
        if choice == "1":
            ...
            if user:
                self.run_command(f"ausearch -ua {user} > {output_dir}/user_activities.log", sudo=True)
            else:
                self.run_command(f"aureport --user > {output_dir}/all_user_activities.log", sudo=True)
                
        elif choice == "3":
            print(f"\n{Fore.YELLOW}Yetki değişiklikleri kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"ausearch -m USER_ACCT -m USER_ROLE_CHANGE > {output_dir}/permission_changes.log", sudo=True)
            
        elif choice == "4":
            print(f"\n{Fore.YELLOW}Parola politikası kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"cat /etc/security/pwquality.conf > {output_dir}/password_policy.txt")
    ...
    def login_monitor(self):
        ...
        if choice == "1":
            ...
            # IP bazlı analiz
            self.run_command("lastb | awk '{print $3}' | sort | uniq -c | sort -nr > " +
                            f"{output_dir}/failed_login_ips.txt", sudo=True)
            
        elif choice == "3":
            print(f"\n{Fore.YELLOW}SSH giriş kayıtları inceleniyor...{Style.RESET_ALL}")
            self.run_command(f"grep 'sshd' /var/log/auth.log > {output_dir}/ssh_logins.log", sudo=True)
            ...
            end_time = time.time() + (float(duration) * 60)
            while time.time() < end_time:
                self.run_command(f"tail -f /var/log/auth.log > {output_dir}/realtime_logins.log", sudo=True)
                time.sleep(1)
    ...
    def service_audit(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"systemctl list-units --type=service --state=running > {output_dir}/running_services.txt")
            
        elif choice == "2":
            print(f"\n{Fore.YELLOW}Servis durumları analiz ediliyor...{Style.RESET_ALL}")
            self.run_command(f"systemctl list-units --type=service --all > {output_dir}/all_services.txt")
            
            # Başarısız servisleri analiz et
            self.run_command("systemctl --failed --type=service > " +
                            f"{output_dir}/failed_services.txt")
            
        elif choice == "3":
            print(f"\n{Fore.YELLOW}Otomatik başlayan servisler kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"systemctl list-unit-files --type=service | grep enabled > {output_dir}/enabled_services.txt")
            
        elif choice == "4":
            print(f"\n{Fore.YELLOW}Servis güvenlik denetimi yapılıyor...{Style.RESET_ALL}")
            services = self.run_command("systemctl list-units --type=service --state=running | awk '{print $1}'").splitlines()
            ...
            with open(f"{output_dir}/service_security_audit.txt", 'w') as f:
                for service in services:
                    if service.endswith('.service'):
                        ...
                        f.write(self.run_command(f"systemctl show {service}") + "\n")
                        # Servis dosya izinlerini kontrol et
                        service_path = self.run_command(f"systemctl show {service} -p FragmentPath")
                        if service_path:
                            f.write(self.run_command(f"ls -l {service_path.split('=')[1]}") + "\n")
    ...
    def password_audit(self):
        ...
        if choice == "1" and self.check_tool("john"):
            ...
            self.run_command(f"john --show /etc/shadow > {output_dir}/password_hashes.txt", sudo=True)
            
        elif choice == "2":
            print(f"\n{Fore.YELLOW}Zayıf parolalar tespit ediliyor...{Style.RESET_ALL}")
            if self.check_tool("john"):
                self.run_command(f"john --single /etc/shadow > {output_dir}/weak_passwords.txt", sudo=True)
                
        elif choice == "3":
            print(f"\n{Fore.YELLOW}Parola politikası kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"cat /etc/pam.d/common-password > {output_dir}/password_policy.txt")
            self.run_command(f"cat /etc/security/pwquality.conf >> {output_dir}/password_policy.txt")
            
        elif choice == "4":
            print(f"\n{Fore.YELLOW}Kullanıcı parola durumları kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"chage -l $(cut -d: -f1 /etc/passwd) > {output_dir}/password_status.txt", sudo=True)
    ...
    def file_permission_check(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"find {path} -type f -perm /4000 > {output_dir}/high_perm_files.txt")
            
        elif choice == "2":
            print(f"\n{Fore.YELLOW}SUID/SGID dosyaları kontrol ediliyor...{Style.RESET_ALL}")
            self.run_command(f"find / -type f -perm /6000 > {output_dir}/suid_sgid_files.txt")
    ...
    def system_hardening(self):
        ...
        if choice == "1" and self.check_tool("lynis"):
            ...
            self.run_command(f"lynis audit system > {output_dir}/security_audit.txt", sudo=True)
            ...
            with open(f"{output_dir}/security_report.txt", 'w') as f:
                ...
                f.write(self.run_command("sestatus") + "\n\n")
                
                # Güvenlik duvarı durumu
                f.write("Güvenlik Duvarı Durumu:\n")
                f.write(self.run_command("firewall-cmd --list-all") + "\n\n")
                
                # Açık portlar
                f.write("Açık Portlar:\n")
                f.write(self.run_command("ss -tuln") + "\n\n")
    ...
    def backup_check(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"find /backup -type f -mtime -{days} > {output_dir}/recent_backups.txt")
            
        elif choice == "2":
            print(f"\n{Fore.YELLOW}Yedekleme durumu kontrol ediliyor...{Style.RESET_ALL}")
            # Yedekleme servislerinin durumu
            self.run_command(f"systemctl status *backup* > {output_dir}/backup_services.txt")
            ...
            with open(f"{output_dir}/backup_report.txt", 'w') as f:
                ...
                f.write(self.run_command("find /backup -type f -mtime -7 -ls") + "\n\n")
                
                # Yedekleme politikası
                f.write("Yedekleme Politikası:\n")
                if os.path.exists("/etc/backup.conf"):
                    f.write(self.run_command("cat /etc/backup.conf") + "\n\n")
                
                # Yedekleme alanı kullanımı
                f.write("Yedekleme Alanı Kullanımı:\n")
                f.write(self.run_command("du -sh /backup/*") + "\n")
    ...
    def encryption_check(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"cryptsetup luksDump /dev/sda* > {output_dir}/luks_headers.txt 2>/dev/null", sudo=True)
            
        elif choice == "2":
            print(f"\n{Fore.YELLOW}LUKS durumu kontrol ediliyor...{Style.RESET_ALL}")
            # Aktif LUKS bağlantıları
            self.run_command(f"dmsetup ls --target crypt > {output_dir}/active_luks.txt", sudo=True)
            
            # LUKS versiyonları
            self.run_command(f"cryptsetup luksDump /dev/mapper/* > {output_dir}/luks_versions.txt 2>/dev/null", sudo=True)
            ...
            with open(f"{output_dir}/encryption_policies.txt", 'w') as f:
                for policy, file in policies.items():
                    f.write(f"\n=== {policy} ===\n")
                    f.write(self.run_command(f"grep {policy} {file}") + "\n")
            ...
            with open(f"{output_dir}/encryption_report.txt", 'w') as f:
                ...
                f.write(self.run_command("lsblk -f") + "\n\n")
                
                # LUKS başlıkları
                f.write("LUKS Başlıkları:\n")
                f.write(self.run_command("cryptsetup luksDump /dev/sda* 2>/dev/null", sudo=True) + "\n\n")
                
                # Şifreleme algoritmaları
                f.write("Kullanılan Şifreleme Algoritmaları:\n")
                f.write(self.run_command("dmsetup table") + "\n")
                ...
                if choice == "4":
                    with open(f"{output_dir}/malfind.txt", 'r') as f:
                        if "suspicious" in f.read():
                            print(f"{Fore.RED}Şüpheli aktivite tespit edildi!{Style.RESET_ALL}")
    ...
    def log_analysis(self):
        ...
        time_range = input("Zaman aralığı (örn: '1 hour ago', 'yesterday'): ") or "today"
        ...
        if choice in commands:
            ...
            with open(f"{output_dir}/analysis_report.txt", 'w') as f:
                ...
                for pattern in patterns:
                    result = self.run_command(f"grep -i {pattern} {commands[choice]} | wc -l")
            ...
            if choice in commands:
                ...
                with open(f"{output_dir}/recovery_report.txt", 'w') as f:
                    ...
                    for ext in ['jpg', 'pdf', 'doc', 'txt', 'zip']:
                        count = self.run_command(f"find {output_dir} -name '*.{ext}' | wc -l")
    ...
    def timeline_analysis(self):
        ...
        if self.check_tool("mactime"):
            ...
            end_date = input("Bitiş tarihi (YYYY-MM-DD) [bugün]: ") or \
                ...
                with open(f"{output_dir}/timeline_report.txt", 'w') as f:
                    ...
                    if choice in ["1", "2"]:
                        ...
                        if os.path.exists(timeline_file):
                            total_events = self.run_command(f"wc -l {timeline_file}").split()[0]
                            f.write(f"Toplam Olay Sayısı: {total_events}\n")

                            # Saat bazlı aktivite dağılımı
                            f.write("\nSaat Bazlı Aktivite Dağılımı:\n")
                            self.run_command(f"awk -F',' '{{print $1}}' {timeline_file} | " + 
                                        "awk -F':' '{print $1}' | sort | uniq -c >> " + 
                                        f"{output_dir}/timeline_report.txt")

                # Görselleştirme (opsiyonel)
                if input("\nZaman çizelgesi görselleştirilsin mi? (e/h): ").lower() == 'e':
                    if self.check_tool("gnuplot"):
                        self.run_command(f"gnuplot -e 'set terminal png; set output \"{output_dir}/timeline.png\"; " + 
    ...
    def network_forensics(self):
        """41. Ağ Adli Analizi"""
        print(f"\n{Fore.CYAN}Ağ Adli Analiz Seçenekleri:{Style.RESET_ALL}")
        print("1. Paket Yakalama Analizi")
        print("2. Akış Analizi") 
        print("3. DNS Trafiği Analizi")
        print("4. SSL/TLS Trafiği Analizi")

        choice = input("\nSeçiminiz (1-4): ")
        output_dir = f"network_forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        os.makedirs(output_dir, exist_ok=True)

        if choice == "1" and self.check_tool("tshark"):
            pcap_file = input("PCAP dosyası: ")
            print(f"\n{Fore.YELLOW}Paket analizi başlatılıyor...{Style.RESET_ALL}")
            # Temel istatistikler
            self.run_command(f"capinfos {pcap_file} > {output_dir}/packet_stats.txt")
            # Protokol hiyerarşisi
            self.run_command(f"tshark -r {pcap_file} -q -z io,phs > {output_dir}/protocol_hierarchy.txt")
            # En aktif IP'ler
            self.run_command(f"tshark -r {pcap_file} -q -z endpoints,ip > {output_dir}/top_ips.txt")

        elif choice == "2" and self.check_tool("argus"):
            pcap_file = input("PCAP dosyası: ")
            print(f"\n{Fore.YELLOW}Akış analizi başlatılıyor...{Style.RESET_ALL}")
            # Akış oluştur
            self.run_command(f"argus -r {pcap_file} -w {output_dir}/flow.argus")
            # Akış analizi
            self.run_command(f"ra -r {output_dir}/flow.argus -s stime flgs proto saddr sport daddr dport > {output_dir}/flow_analysis.txt")

        elif choice == "3" and self.check_tool("dig"):
            print(f"\n{Fore.YELLOW}DNS trafiği analizi başlatılıyor...{Style.RESET_ALL}")
            # DNS sorgularını analiz et
            log_file = input("DNS log dosyası: ")
            self.run_command(f"tshark -r {log_file} -Y 'dns' -T fields -e dns.qry.name -e dns.resp.name > {output_dir}/dns_queries.txt")
            
        elif choice == "4" and self.check_tool("ssldump"):
            pcap_file = input("PCAP dosyası: ")
            key_file = input("SSL key dosyası (opsiyonel): ")
            print(f"\n{Fore.YELLOW}SSL/TLS trafiği analizi başlatılıyor...{Style.RESET_ALL}")
            cmd = f"ssldump -r {pcap_file}"
            if key_file:
                cmd += f" -k {key_file}"
            self.run_command(f"{cmd} > {output_dir}/ssl_analysis.txt")

def database_forensics(self):
    """42. Veritabanı Adli Analizi"""
    print(f"\n{Fore.CYAN}Veritabanı Adli Analiz Seçenekleri:{Style.RESET_ALL}")
    print("1. SQLite Analizi")
    print("2. MySQL Log Analizi")
    print("3. PostgreSQL Analizi")
    print("4. Veritabanı Dosya Kurtarma")

    choice = input("\nSeçiminiz (1-4): ")
    output_dir = f"db_forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)

    if choice == "1" and self.check_tool("sqlite3"):
        db_file = input("SQLite veritabanı dosyası: ")
        print(f"\n{Fore.YELLOW}SQLite analizi başlatılıyor...{Style.RESET_ALL}")
        # Şema analizi
        self.run_command(f"sqlite3 {db_file} .schema > {output_dir}/sqlite_schema.txt")
        # Tablo bilgileri
        self.run_command(f"sqlite3 {db_file} '.tables' > {output_dir}/sqlite_tables.txt")
        # Silinmiş kayıtlar
        self.run_command(f"strings {db_file} | grep -i 'delete\|insert\|update' > {output_dir}/sqlite_deleted.txt")

    elif choice == "2" and self.check_tool("mysqlbinlog"):
        log_file = input("MySQL binary log dosyası: ")
        print(f"\n{Fore.YELLOW}MySQL log analizi başlatılıyor...{Style.RESET_ALL}")
        # Binary log analizi
        self.run_command(f"mysqlbinlog {log_file} > {output_dir}/mysql_binlog.txt")
        # Sorgu analizi
        self.run_command(f"grep -i 'insert\|update\|delete' {log_file} > {output_dir}/mysql_queries.txt")

    elif choice == "3" and self.check_tool("pg_dump"):
        db_name = input("PostgreSQL veritabanı adı: ")
        print(f"\n{Fore.YELLOW}PostgreSQL analizi başlatılıyor...{Style.RESET_ALL}")
        # Şema dökümü
        self.run_command(f"pg_dump -s {db_name} > {output_dir}/postgres_schema.sql")
        # Log analizi
        self.run_command(f"grep -i 'error\|warning' /var/log/postgresql/* > {output_dir}/postgres_logs.txt")

    elif choice == "4" and self.check_tool("photorec"):
        disk_image = input("Disk imaj dosyası: ")
        print(f"\n{Fore.YELLOW}Veritabanı dosya kurtarma başlatılıyor...{Style.RESET_ALL}")
        # PhotoRec ile kurtarma
        self.run_command(f"photorec /d {output_dir}/recovered /log {disk_image}")

def email_forensics(self):
    """43. E-posta Adli Analizi"""
    print(f"\n{Fore.CYAN}E-posta Adli Analiz Seçenekleri:{Style.RESET_ALL}")
    print("1. E-posta Başlık Analizi")
    print("2. MIME Analizi")
    print("3. E-posta Ekleri Analizi")
    print("4. E-posta Sunucu Logları Analizi")

    choice = input("\nSeçiminiz (1-4): ")
    output_dir = f"email_forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)

    if choice == "1":
        email_file = input("E-posta dosyası: ")
        print(f"\n{Fore.YELLOW}E-posta başlık analizi başlatılıyor...{Style.RESET_ALL}")
        # Başlık analizi
        self.run_command(f"formail -X From -X To -X Subject -X Date < {email_file} > {output_dir}/headers.txt")
        # SPF ve DKIM kontrolü
        self.run_command(f"grep -i 'spf\|dkim' {email_file} > {output_dir}/authentication.txt")

    elif choice == "2" and self.check_tool("munpack"):
        email_file = input("E-posta dosyası: ")
        print(f"\n{Fore.YELLOW}MIME analizi başlatılıyor...{Style.RESET_ALL}")
        # MIME yapısını analiz et
        self.run_command(f"munpack -t {email_file} > {output_dir}/mime_structure.txt")

    elif choice == "3":
        email_dir = input("E-posta dizini: ")
        print(f"\n{Fore.YELLOW}E-posta ekleri analizi başlatılıyor...{Style.RESET_ALL}")
        # Ekleri çıkar
        attachments_dir = f"{output_dir}/attachments"
        os.makedirs(attachments_dir, exist_ok=True)
        self.run_command(f"ripmime -i {email_dir} -d {attachments_dir}")
        # Dosya türlerini analiz et
        self.run_command(f"file {attachments_dir}/* > {output_dir}/attachment_types.txt")

    elif choice == "4":
        log_file = input("Mail sunucu log dosyası: ")
        print(f"\n{Fore.YELLOW}E-posta sunucu logları analizi başlatılıyor...{Style.RESET_ALL}")
        # Log analizi
        self.run_command(f"grep 'connect from\|disconnect from' {log_file} > {output_dir}/connections.txt")
        self.run_command(f"grep 'status=sent\|status=bounced' {log_file} > {output_dir}/delivery_status.txt")

def browser_forensics(self):
    """44. Tarayıcı Adli Analizi"""
    print(f"\n{Fore.CYAN}Tarayıcı Adli Analiz Seçenekleri:{Style.RESET_ALL}")
    print("1. Chrome Analizi")
    print("2. Firefox Analizi")
    print("3. Safari Analizi")
    print("4. Tarayıcı Artefaktları")

    choice = input("\nSeçiminiz (1-4): ")
    output_dir = f"browser_forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)

    if choice == "1":
        chrome_dir = input("Chrome profil dizini: ")
        print(f"\n{Fore.YELLOW}Chrome analizi başlatılıyor...{Style.RESET_ALL}")
        # Geçmişi analiz et
        self.run_command(f"sqlite3 {chrome_dir}/History 'SELECT url, title, last_visit_time FROM urls' > {output_dir}/chrome_history.txt")
        # İndirmeleri analiz et
        self.run_command(f"sqlite3 {chrome_dir}/History 'SELECT * FROM downloads' > {output_dir}/chrome_downloads.txt")
        # Çerezleri analiz et
        self.run_command(f"sqlite3 {chrome_dir}/Cookies 'SELECT * FROM cookies' > {output_dir}/chrome_cookies.txt")

    elif choice == "2":
        firefox_dir = input("Firefox profil dizini: ")
        print(f"\n{Fore.YELLOW}Firefox analizi başlatılıyor...{Style.RESET_ALL}")
        # Places veritabanını analiz et
        self.run_command(f"sqlite3 {firefox_dir}/places.sqlite 'SELECT url, title, last_visit_date FROM moz_places' > {output_dir}/firefox_history.txt")
        # İndirmeleri analiz et
        self.run_command(f"sqlite3 {firefox_dir}/downloads.sqlite 'SELECT * FROM moz_downloads' > {output_dir}/firefox_downloads.txt")

    elif choice == "3":
        safari_dir = input("Safari dizini: ")
        print(f"\n{Fore.YELLOW}Safari analizi başlatılıyor...{Style.RESET_ALL}")
        # Geçmişi analiz et
        self.run_command(f"plutil -convert xml1 -o {output_dir}/safari_history.txt {safari_dir}/History.plist")
        # İndirmeleri analiz et
        self.run_command(f"plutil -convert xml1 -o {output_dir}/safari_downloads.txt {safari_dir}/Downloads.plist")

    elif choice == "4":
        user_dir = input("Kullanıcı dizini: ")
        print(f"\n{Fore.YELLOW}Tarayıcı artefaktları analizi başlatılıyor...{Style.RESET_ALL}")
        # Cache dosyalarını ara
        self.run_command(f"find {user_dir} -type f -name 'Cache*' > {output_dir}/browser_cache.txt")
        # Bookmark dosyalarını ara
        self.run_command(f"find {user_dir} -type f -name 'Bookmarks*' > {output_dir}/browser_bookmarks.txt")
        # Tarayıcı profillerini ara
        self.run_command(f"find {user_dir} -type d -name '*Profile*' > {output_dir}/browser_profiles.txt")

def mobile_forensics(self):
    """45. Mobil Cihaz Adli Analizi"""
    print(f"\n{Fore.CYAN}Mobil Cihaz Adli Analiz Seçenekleri:{Style.RESET_ALL}")
    print("1. Android Analizi")
    print("2. iOS Analizi")
    print("3. Mobil Uygulama Analizi")
    print("4. Mobil İmaj Analizi")

    choice = input("\nSeçiminiz (1-4): ")
    output_dir = f"mobile_forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)

    if choice == "1" and self.check_tool("adb"):
        print(f"\n{Fore.YELLOW}Android analizi başlatılıyor...{Style.RESET_ALL}")
        # Cihaz bilgilerini al
        self.run_command(f"adb shell getprop > {output_dir}/device_info.txt")
        # Uygulama listesi
        self.run_command(f"adb shell pm list packages > {output_dir}/installed_apps.txt")
        # Sistem logları
        self.run_command(f"adb logcat -d > {output_dir}/system_logs.txt")
        # Dosya sistemi dökümü
        self.run_command(f"adb pull /sdcard/ {output_dir}/sdcard/")

    elif choice == "2" and self.check_tool("ideviceinfo"):
        print(f"\n{Fore.YELLOW}iOS analizi başlatılıyor...{Style.RESET_ALL}")
        # Cihaz bilgilerini al
        self.run_command(f"ideviceinfo > {output_dir}/device_info.txt")
        # Uygulama listesi
        self.run_command(f"ideviceinstaller -l > {output_dir}/installed_apps.txt")
        # Sistem logları
        self.run_command(f"idevicesyslog > {output_dir}/system_logs.txt")
        # Yedekleme
        self.run_command(f"idevicebackup2 backup {output_dir}/backup/")

    elif choice == "3":
        apk_file = input("APK/IPA dosyası: ")
        print(f"\n{Fore.YELLOW}Mobil uygulama analizi başlatılıyor...{Style.RESET_ALL}")
        if apk_file.endswith('.apk'):
            # APK analizi
            self.run_command(f"apktool d {apk_file} -o {output_dir}/decompiled")
            self.run_command(f"dex2jar {apk_file} --output {output_dir}/classes.jar")
            self.run_command(f"jd-gui {output_dir}/classes.jar")
        else:
            # IPA analizi
            self.run_command(f"unzip {apk_file} -d {output_dir}/extracted")
            self.run_command(f"strings {output_dir}/extracted/Payload/*.app/executable > {output_dir}/strings.txt")

    elif choice == "4" and self.check_tool("autopsy"):
        image_file = input("Mobil cihaz imaj dosyası: ")
        print(f"\n{Fore.YELLOW}Mobil imaj analizi başlatılıyor...{Style.RESET_ALL}")
        # İmaj analizi
        self.run_command(f"mmls {image_file} > {output_dir}/partition_info.txt")
        self.run_command(f"fls -r {image_file} > {output_dir}/file_listing.txt")
        # Autopsy ile analiz
        self.run_command(f"autopsy -d {image_file} -o {output_dir}/autopsy")
    ...
    def password_cracking(self):
        ...
        if self.check_tool("hashcat"):
            hash_file = input("Hash dosyası: ")
            
            # Hash türünü otomatik tespit
            hash_type = self.run_command(f"hashid {hash_file}")
            print(f"\n{Fore.YELLOW}Tespit Edilen Hash Türleri:{Style.RESET_ALL}\n{hash_type}")
            
            hash_mode = input("Hash modu (-m) [0]: ") or "0"
            
            if choice == "1":
                wordlist = input("Wordlist dosyası [rockyou.txt]: ") or "/usr/share/wordlists/rockyou.txt"
            ...
            if os.path.exists(f"{output_dir}/cracked.txt"):
                with open(f"{output_dir}/cracked.txt", 'r') as f:
                    cracked = f.read()
    ...
    def exploit_search(self):
        ...
        if self.check_tool("searchsploit"):
            if choice == "1":
                ...
                platform_map = {"1": "windows", "2": "linux", "3": "android", "4": "osx"}
                ...
                if platform: cmd += f" -p {platform}"
                if type: cmd += f" -t {type}"
    ...
    def web_fuzzing(self):
        ...
        if self.check_tool("wfuzz"):
            ...
            if choice == "1":
                wordlist = input("Wordlist [common.txt]: ") or "/usr/share/wfuzz/wordlist/general/common.txt"
                ...
                if extensions:
                    cmd += f" -z file,{wordlist} -z list,{extensions} -u {url}/FUZZ.FUZ2Z"
                else:
                    cmd += f" -u {url}/FUZZ"
    ...
    def xss_scan(self):
        ...
        if self.check_tool("xsser"):
            ...
            if choice == "1":
                ...
                cmd = f"xsser --url {url} --auto --Fp --Cw --Str --DOM -v --reverse-check"
            ...
            if input("\nHTML rapor oluşturulsun mu? (e/h): ").lower() == 'e':
                self.run_command(f"xsser --url {url} --xml-file={output_dir}/xss_results.xml")
            ...
            if choice in commands:
                ...
                with open(f"{output_dir}/analysis_report.txt", 'w') as f:
                    ...
                    if os.path.exists(f"{output_dir}/functions.txt"):
                        func_count = self.run_command(f"wc -l {output_dir}/functions.txt").split()[0]
                        f.write(f"Fonksiyon Sayısı: {func_count}\n")

                    if os.path.exists(f"{output_dir}/imports.txt"):
                        import_count = self.run_command(f"wc -l {output_dir}/imports.txt").split()[0]
                        f.write(f"İçe Aktarma Sayısı: {import_count}\n")

                    if os.path.exists(f"{output_dir}/strings.txt"):
                        string_count = self.run_command(f"wc -l {output_dir}/strings.txt").split()[0]
        ...
        if self.check_tool("cuckoo"):
            ...
            if choice in commands:
                ...
                if os.path.exists("/rules/malware"):
                    ...
                    self.run_command(f"yara -r /rules/malware {file} > {output_dir}/yara_matches.txt")
    ...
    def threat_hunting(self):
        ...
        if choice == "1" and self.check_tool("yara"):
            ...
            if rule_choice == "1":
                self.run_command(f"yara -r /rules/* {target_path} > {output_dir}/yara_results.txt")
            elif rule_choice == "2":
                rule_file = input("Kural dosyası: ")
                self.run_command(f"yara -r {rule_file} {target_path} > {output_dir}/custom_yara_results.txt")
            elif rule_choice == "3":
                # Online YARA kurallarını güncelle
                self.run_command("git clone https://github.com/Yara-Rules/rules.git /tmp/yara-rules")
                self.run_command(f"yara -r /tmp/yara-rules/malware/* {target_path} > {output_dir}/updated_yara_results.txt")

        elif choice == "2":
            print("\nŞüpheli Süreç Analizi yapılıyor...")
            # Şüpheli süreçleri kontrol et
            self.run_command(f"ps aux | grep -i 'crypto\\|miner\\|malware' > {output_dir}/suspicious_processes.txt")
            self.run_command(f"lsof -i > {output_dir}/network_connections.txt")
            
        elif choice == "3":
            print("\nIOC Taraması yapılıyor...")
            # IOC'leri kontrol et
            self.run_command(f"find / -type f -exec md5sum {{}} \\; > {output_dir}/file_hashes.txt")
        ...
        with open(f"{output_dir}/analysis_report.txt", 'w') as f:
            ...
            if os.path.exists(f"{output_dir}/yara_results.txt"):
                yara_hits = self.run_command(f"wc -l {output_dir}/yara_results.txt").split()[0]
                f.write(f"YARA Eşleşmeleri: {yara_hits}\n")
                
            if os.path.exists(f"{output_dir}/suspicious_processes.txt"):
                suspicious = self.run_command(f"wc -l {output_dir}/suspicious_processes.txt").split()[0]
    ...
    def osint_gathering(self):
        ...
        if choice == "1" and self.check_tool("theharvester"):
            ...
            self.run_command(f"sherlock {username} --output {output_dir}/social_media.txt")
    ...
    def reverse_engineering(self):
        ...
        if choice == "1" and self.check_tool("ghidra"):
            ...
            self.run_command(f"ghidra -import {target_file} -postScript AnalyzeHeadless.java {project_name}")
        ...
        with open(f"{output_dir}/analysis_report.txt", 'w') as f:
            ...
            if os.path.exists(f"{output_dir}/functions.txt"):
                ...
                f.write(self.run_command(f"cat {output_dir}/functions.txt"))
    ...
    def steganography(self):
        ...
        if choice == "1" and self.check_tool("steghide"):
            ...
            self.run_command(f"stegolsb steglsb -r -i {file} -o {output_dir}/lsb_output")
    ...
    def crypto_analysis(self):
        ...
        if choice == "1" and self.check_tool("cryptsetup"):
            ...
            self.run_command(f"openssl s_client -connect {host}:{port} -showcerts > {output_dir}/ssl_analysis.txt")
    ...
    def container_security(self):
        ...
        if choice == "1" and self.check_tool("docker"):
            ...
            self.run_command(f"docker scan $(docker ps -q) > {output_dir}/container_scan.txt")
            # Docker daemon güvenlik kontrolü
            self.run_command(f"docker info --format '{{.SecurityOptions}}' > {output_dir}/security_options.txt")
            # Docker compose güvenlik kontrolü
            self.run_command(f"docker-compose config --services > {output_dir}/compose_services.txt")
            ...
            images = self.run_command("docker images --format '{{.Repository}}:{{.Tag}}'").splitlines()
            for image in images:
                self.run_command(f"trivy image {image} > {output_dir}/image_scan_{image.replace(':', '_')}.txt")
            ...
            self.run_command(f"kubectl get psp > {output_dir}/pod_security_policies.txt")
            # Network politikaları
            self.run_command(f"kubectl get networkpolicies --all-namespaces > {output_dir}/network_policies.txt")
        ...
        with open(f"{output_dir}/security_report.txt", 'w') as f:
            ...
            if self.check_tool("docker"):
                ...
                f.write(self.run_command("docker version") + "\n")
                f.write(self.run_command("docker info") + "\n\n")
    ...
    def compliance_check(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"find / -type f -exec grep -l -i 'password\\|email\\|phone' {{}} \\; > {output_dir}/personal_data_files.txt")
            # Log retention kontrolü
            self.run_command(f"find /var/log -type f -mtime +365 > {output_dir}/old_logs.txt")
            # Veri şifreleme kontrolü
            self.run_command(f"grep -r 'ssl\\|tls\\|encrypt' /etc/ > {output_dir}/encryption_config.txt")
            ...
            self.run_command(f"apt list --upgradable > {output_dir}/security_updates.txt")
            # Antivirüs kontrolü
            ...
            self.run_command(f"find /backup -type f -mtime -7 > {output_dir}/recent_backups.txt")
            # Güvenlik politikaları
            self.run_command(f"find /etc/security -type f -exec cat {{}} \\; > {output_dir}/security_policies.txt")
            ...
            self.run_command(f"oscap xccdf eval --profile xccdf_org.ssgproject.content_profile_cis \
    ...
    def security_audit(self):
        ...
        if choice == "1" and self.check_tool("lynis"):
            ...
            self.run_command(f"lynis audit system --pentest --verbose > {output_dir}/system_audit.txt", sudo=True)
            # AIDE ile dosya bütünlük kontrolü
            self.run_command(f"aide --check > {output_dir}/integrity_check.txt", sudo=True)
            # Rootkit kontrolü
            self.run_command(f"rkhunter --check --skip-keypress > {output_dir}/rootkit_check.txt", sudo=True)

        elif choice == "2":
            print(f"\n{Fore.YELLOW}Ağ güvenlik denetimi başlatılıyor...{Style.RESET_ALL}")
            # Açık port taraması
            self.run_command(f"nmap -sS -sV localhost > {output_dir}/port_scan.txt", sudo=True)
            # Güvenlik duvarı kuralları
            self.run_command(f"iptables -L -n -v > {output_dir}/firewall_rules.txt", sudo=True)
            # Ağ bağlantıları
            ...
            # Servis durumları
            self.run_command(f"systemctl list-units --type=service > {output_dir}/service_status.txt")
            # Apache/Nginx yapılandırma kontrolü
            if self.check_tool("apache2"):
                self.run_command(f"apache2ctl -t -D DUMP_VHOSTS > {output_dir}/apache_config.txt")
            ...
            # Kullanıcı hakları
            self.run_command(f"for user in $(cut -d: -f1 /etc/passwd); do groups $user; done > {output_dir}/user_groups.txt")
            # Sudo yapılandırması
            self.run_command(f"cat /etc/sudoers > {output_dir}/sudo_config.txt", sudo=True)
            # SSH yapılandırması
        ...
        print(f"{Fore.GREEN}Düşük Riskli Bulgular: {len(risk_levels['LOW'])}{Style.RESET_ALL}")
    ...
    def system_backup(self):
        ...
        if choice == "1":
            ...
            self.run_command(f"tar -czpf {output} --exclude=/proc --exclude=/tmp --exclude=/backup --exclude=/mnt --exclude=/dev --exclude=/sys --exclude=/run --exclude=/media --exclude=/var/log --exclude=/var/cache/apt/archives /", sudo=True)
    ...
    def show_installed_tools(self):
        ...
        for tool in self.required_tools:
            sys.stdout.write(f"\r{Fore.YELLOW}Kontrol ediliyor: {tool}{Style.RESET_ALL}")
            sys.stdout.flush()
        ...
        if installed_tools:
            ...
            for tool, version in installed_tools:
                truncated_version = version[:50] + '...' if len(version) > 50 else version
                print(f"{Fore.GREEN}✓ {tool:<20}{Style.RESET_ALL} - {truncated_version}")
        ...
        if missing_tools:
            if input(f"\n{Fore.YELLOW}Eksik araçları kurmak ister misiniz? (e/h): {Style.RESET_ALL}").lower() == 'e':
                for tool in missing_tools:
                    print(f"\n{Fore.CYAN}Yükleniyor: {tool}{Style.RESET_ALL}")
                    if self.install_package(tool):
                        print(f"{Fore.GREEN}{tool} başarıyla yüklendi.{Style.RESET_ALL}")
                    else:
...
╚══════╝╚═╝╚═╝     ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝
                ...
                options = {
                    ...
                    16: ("Ağ Taraması", self.ssl_scan),
                    17: ("Ağ Güvenlik Taraması", self.wifi_scan),
                    18: ("Sistem Güvenlik Taraması", self.packet_capture),
                    19: ("Güvenlik Duvarı Kontrolü", self.arp_scan),
                    20: ("Ağ Güvenliği Kontrolü", self.dns_enum),
                    ...
                    41: ("Ağ Adli Analizi", self.network_forensics),
                    42: ("Veritabanı Adli Analizi", self.database_forensics),
                    43: ("E-posta Adli Analizi", self.email_forensics), 
                    44: ("Tarayıcı Adli Analizi", self.browser_forensics),
                    45: ("Mobil Cihaz Adli Analizi", self.mobile_forensics),
                ...
                try:
                    ...
                    if choice in options:
                        ...
                        print(f"{Fore.RED}Geçersiz seçim! Lütfen 1-64 arası bir sayı girin.{Style.RESET_ALL}")
                except ValueError:
                    print(f"{Fore.RED}Lütfen geçerli bir sayı girin.{Style.RESET_ALL}")
                except KeyboardInterrupt:
